...
import com.example.account.service.account.IAccountService;
import com.example.account.utils.Email;
...
@WebServlet(name = "AccountServlett", value = "/account")
        ...
        if (errMap.isEmpty()) {
            ...
            if (!accountService.checkUserName(userName)) {
                if (password.equals(confirmPassword)) {
                    ...
                    if (rowUpdate) {
                        request.setAttribute("userName", userName);
                        requestDispatcher = request.getRequestDispatcher("account/create.jsp");
                    } else {
